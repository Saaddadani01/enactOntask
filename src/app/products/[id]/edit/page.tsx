...
function EditProduct({ params }: { params: { id: string } }) {
  ...
  const {
    ...
    onSubmit: async (values, actions) => {
      const updatedProduct = {
        ...
        brands: values.brands ? values.brands.map(option => option.value) : [],
        categories: values.categories ? values.categories.map(option => option.value) : [],
        gender: values.gender,
        occasion: values.occasion ? values.occasion.map(option => option.value).join(',') : '',
        image_url: values.image_url ||initialImageUrl
      };
      try {
        await updateProduct(id, updatedProduct);
        alert("Product updated successfully!");
        router.push('/products'); // Redirect or show success message
      } catch (error) {
  ...
  useEffect(() => {
    ...
    (async function () {
      ...
      setInitialImageUrl(productArr.image_url);
      const product = {
