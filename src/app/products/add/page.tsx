...
import { addSingleProduct } from "@/actions/productActions"
...
function AddProduct() {
  ...
  const {
    ...
    onSubmit: async (values: any, actions) => {
      ...
      const mappedOccasion = values.occasion ? values.occasion.map(o => o.value).join(",") : "";
      const mappedBrands = values.brands ? values.brands.map(b => b.value).join(",") : "";
      console.log("mappedOccasion----",mappedOccasion);
      console.log("mappedBrands----",mappedBrands);
      const old_price= parseFloat(values.old_price);
      ...
      try{
       await addSingleProduct(productData);
        ...
        resetForm();
        router.push("/products"); // Redirect or perform other actions as needed
      } catch (error) {
